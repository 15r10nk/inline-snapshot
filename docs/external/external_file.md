...
`external_file()` is a more low-level solution than `external()`.
It accepts one argument, which is the path to the file where your external object should be stored.
It will only *create* or *fix* the given files and never *trim* unused files.
...
It can be used to generate some files in your project.
...
def test_files():
    for file in root.rglob("*.md", format=".txt"):
        ...
        # eval_code_blocks is a custom function which could run your examples a project specific way and store the output in the documentation.
