...
The following feature is available for \[insider\](https://15r10nk.github.io/inline-snapshot/(customize preview)/insiders/index.md) only and requires cpython>=3.11 to generate code. The generated code can be used with every python version.
...
You can then run `pytest` to create the value or use the "run test" button in \[PyCharm\](https://15r10nk.github.io/inline-snapshot/(customize preview)/pycharm/index.md), which replaces `...` by default with the correct value.
