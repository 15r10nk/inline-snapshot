You can use almost any python data type and also complex values like `datetime.date`, because `repr()` is used to convert the values to source code. The default `__repr__()` behaviour can be [customized](https://15r10nk.github.io/inline-snapshot/0.32/customize_repr/index.md). It might be necessary to import the right modules to match the `repr()` output.
...
1. The code is generated with:
   ...
   - `repr(value)` (which can be [customized](https://15r10nk.github.io/inline-snapshot/0.32/customize_repr/index.md))
   - or a special internal implementation for container types to support [unmanaged snapshot values](https://15r10nk.github.io/inline-snapshot/0.32/eq_snapshot/#unmanaged-snapshot-values). This can currently not be customized.
